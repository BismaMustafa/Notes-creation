...
//         await axios.put(`http://localhost:3002/api/notes/${editNoteId}`, updatedNote);
//         setIsEditing(false);
//         setEditNoteId(null); // Reset the edit state
//       } else {
//         // Create a new note in MongoDB
//         updatedNote.createdAt = new Date();
//         await axios.post("http://localhost:3002/api/notes", updatedNote);
//       }
...
//     console.log("Editing note:", note); // Check if data is correct
//     setNote({
//       title: note.title,
...
const NotesForm = () => {
  ...
  return (
    <div className="p-8 max-w-2xl mx-auto bg-gradient-to-r from-blue-500 to-teal-500 shadow-lg rounded-lg">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div>
          ...
          <input
            ...
            value={note.title}
            onChange={handleChange}
            className="w-full px-4 py-3 mt-2 text-gray-900 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-300"
            required
          />
        </div>
        <div>
          <label className="block text-lg font-semibold text-white">Content</label>
          <textarea
            name="content"
            value={note.content}
            onChange={handleChange}
        ...
        <div>
          <label className="block text-lg font-semibold text-white">Subject</label>
          <input
            type="text"
            name="subject"
            value={note.subject}
        ...
        <div>
          <label className="block text-lg font-semibold text-white">Created By</label>
          <input
            type="text"
            name="createdBy"
            value={note.createdBy}
        ...
        <div>
          <label className="block text-lg font-semibold text-white">Collaborators</label>
          {note.collaborators.map((collaborator, index) => (
            <div key={index} className="flex items-center mb-2">
              <input
                type="text"
          ...
          <button
            type="button"
            onClick={handleAddCollaborator}
            className="bg-gray-300 text-gray-800 px-4 py-2 rounded-lg mt-2 hover:bg-gray-400"
          >
            Add Collaborator
          </button>
        </div>
        <div>
          <label className="block text-lg font-semibold text-white">Collaborators List</label>
          <ul className="list-disc pl-5 text-white">
            {note.collaborators.map((collaborator, index) => (
              <li
                key={index}
        ...
        <button
          type="submit"
          className="w-full py-3 bg-teal-500 text-white text-lg rounded-lg hover:bg-teal-600 focus:outline-none"
        >
          {isEditing ? "Update Note" : "Create Note"}
        </button>
      </form>
      {statusMessage && (
        <p className="mt-4 text-center text-white text-lg">{statusMessage}</p>
      )}
      <div className="mt-8">
        <h2 className="text-2xl font-semibold text-white mb-6">Notes</h2>
        {notes.length > 0 ? (
          notes.map((note) => (
            <div key={note._id} className="border p-6 rounded-lg mb-4 bg-white shadow-md">
